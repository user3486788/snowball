...
my $username = "user3486788";  # Set the username
# Get current date in YYYY-MM-DD HH:MM:SS format
my $current_date = strftime("%Y-%m-%d %H:%M:%S", localtime);
# Alternatively, use a fixed timestamp for reproducibility
# my $current_date = "2025-07-19 12:39:05";  # Set the timestamp
...
create_amalgamation($output_dir, \@snowball_sources, \@snowball_headers,$current_date, $username);
...
if (-f "$output_dir/amalgamation/libstemmer_amalgamation.c") {
    ...
    # Create C++ Builder project for amalgamation
    print "Creating C++ Builder project for amalgamation...\n";
    my $cb_script_path = File::Spec->catfile($FindBin::Bin, "create_cb_project.pl");
    my $cb_command = "perl \"$cb_script_path\" ";
    $cb_command .= "--output=\"$output_dir\" ";
    $cb_command .= "--input=\"libstemmer_amalgamation.c\" ";
    
    print "Executing: $cb_command\n";
    system($cb_command) == 0 or warn "Failed to create C++ Builder project for amalgamation: $?";
}
...
print "C++ Builder project has been created in the $output_dir/amalgamation directory.\n";
